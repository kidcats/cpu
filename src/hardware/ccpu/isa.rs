...
use core::{fmt};
use std::{collections::{HashMap, btree_set::Union}, default};
...
struct Core{
    ...
    eax:u32,ebx:u32,ecx:u32,edx:u32,esi:u32,edi:u32,ebp:u32,esp:u32,
    ax:u16,bx:u16,cx:u16,dx:u16,si:u16,di:u16,bp:u16,sp:u16,
    al:u8,ah:u8,bl:u8,bh:u8,cl:u8,ch:u8,dl:u8,dh:u8,
    sil:u8,sih:u8,dil:u8,dih:u8,bpl:u8,bph:u8,spl:u8,sph:u8,
    r8d:u32,r9d:u32,r10d:u32,r11d:u32,r12d:u32,r13d:u32,r14d:u32,r15d:u32,
    r8w:u16,r9w:u16,r10w:u16,r11w:u16,r12w:u16,r13w:u16,r14w:u16,r15w:u16,
    r8b:u8,r9b:u8,r10b:u8,r11b:u8,r12b:u8,r13b:u8,r14b:u8,r15b:u8
}

impl  Core{
    fn new() -> Core{
        let regs = (
            RAX_REG{rax:0x0000_0000_0000_0000}, 
            RBX_REG{rbx:0x0000_0000_0000_0000},
            RCX_REG{rcx:0x0000_0000_0000_0000}, 
            RDX_REG{rdx:0x0000_0000_0000_0000}, 
            RSI_REG{rsi:0x0000_0000_0000_0000},
            RDI_REG{rdi:0x0000_0000_0000_0000}, 
            RBP_REG{rbp:0x0000_0000_0000_0000}, 
            RSP_REG{rsp:0x0000_0000_0000_0000}, 
            R8_REG{r8:0x0000_0000_0000_0000}, 
            R9_REG{r9:0x0000_0000_0000_0000}, 
            R10_REG{r10:0x0000_0000_0000_0000}, 
            R11_REG{r11:0x0000_0000_0000_0000}, 
            R12_REG{r12:0x0000_0000_0000_0000}, 
            R13_REG{r13:0x0000_0000_0000_0000}, 
            R14_REG{r14:0x0000_0000_0000_0000}, 
            R15_REG{r15:0x0000_0000_0000_0000} );
        unsafe{Core { 
            rax: {regs.0.rax}, rbx: {regs.1.rbx}, rcx: {regs.2.rcx}, rdx: {regs.3.rdx}, 
            rsi: {regs.4.rsi}, rdi: {regs.5.rdi}, rbp: {regs.6.rbp}, rsp: {regs.7.rsp}, 
            r8: {regs.8.r8}, r9: {regs.9.r9}, r10: {regs.10.r10}, r11: {regs.11.r11}, 
            r12: {regs.12.r12}, r13: {regs.13.r13}, r14: {regs.14.r14}, r15: {regs.15.r15}, 
            eax: {regs.0.eax}, ebx: {regs.1.ebx}, ecx: {regs.2.ecx}, edx: {regs.3.edx}, 
            esi: {regs.4.esi}, edi: {regs.5.edi}, ebp: {regs.6.ebp}, esp: {regs.7.esp}, 
            ax: {regs.0.ax}, bx: {regs.1.bx}, cx: {regs.2.cx}, dx: {regs.3.dx}, 
            si: {regs.4.si}, di: {regs.5.di}, bp: {regs.6.bp}, sp: {regs.7.sp}, 
            al: {regs.0.inner.al}, ah: {regs.0.inner.ah}, bl: {regs.1.inner.bl}, bh: {regs.1.inner.bh}, 
            cl: {regs.2.inner.cl}, ch: {regs.2.inner.ch}, dl: {regs.3.inner.dl}, dh: {regs.3.inner.dh}, 
            sil: {regs.4.inner.sil}, sih: {regs.4.inner.sih}, dil: {regs.5.inner.dil}, dih: {regs.5.inner.dih},
             bpl: {regs.6.inner.bpl}, bph: {regs.6.inner.bph}, spl: {regs.7.inner.spl}, sph: {regs.7.inner.sph}, 
            r8d: {regs.8.r8d}, r9d: {regs.9.r9d}, r10d: {regs.10.r10d}, r11d: {regs.11.r11d}, 
            r12d: {regs.12.r12d}, r13d: {regs.13.r13d}, r14d: {regs.14.r14d}, r15d: {regs.15.r15d}, 
            r8w: {regs.8.r8w}, r9w: {regs.9.r9w}, r10w: {regs.10.r10w}, r11w: {regs.11.r11w}, 
            r12w: {regs.12.r12w}, r13w: {regs.13.r13w}, r14w: {regs.14.r14w}, r15w: {regs.15.r15w}, 
            r8b: {regs.8.r8b}, r9b: {regs.9.r9b}, r10b: {regs.10.r10b}, r11b: {regs.11.r11b}, 
            r12b: {regs.12.r12b}, r13b: {regs.13.r13b}, r14b: {regs.14.r14b}, r15b: {regs.15.r15b} 
        }
        }
    }
}
...
fn parse_mm_ist(str:&str,core:&Core)->u64{
    ...
    let temp = reg2value(r1.as_str(), core).unwrap() + reg2value(r2.as_str(), core).unwrap() * scal_temp;
    let temp2 = hex_str2i(imm.as_str());
    icalu(temp2, temp)
}

fn parse_od_type(str : &str,core : &Core) -> Option<OD>{
    if str.contains("(") { // 如果有括号，则一定是内存型
        // 内存型一共有9种，但是大体上还是 a(reg1,reg2,scal)这种类型
        return  Some(OD::M_REG(parse_mm_ist(str,core)));
    }else if str.starts_with("$") { // 以$开头，则一定是立即数地址型
        return Some(OD::IMM(hex_str2i(str)));
    }else if str.starts_with("%") { // 以%开头，则是寄存器型
        let od =  match str.as_ref() {
...
mod tests {
    ...
    use std::i64;
    use super::{Inst, parse_inst_type, parse_od_type,Core};
    ...
    fn op_works() {
        ...
        let inst1 = String::from("mov %rbp %rsp");
        ...
        // };
        let l2 : Vec<&str> = "-0xfc(%eax,%edx,5)".split(&['(', ')', ','][..]).collect();
        let a = hex_str2i(l2[0]);
        let reg1 = reg2value(l2[1], &core).unwrap();
        let reg2 = reg2value(l2[2], &core).unwrap();
        let scal = hex_str2u(l2[3]);
        let result = icalu(a,reg1+1000+reg2*scal);
        println!("{:?}",result);
        // let result = a + reg1 + reg2 * scal;
        // assert_eq!(result,-0x63);
    }
}
